...
!read data in input
!==================
      open(10,file='achbond_1pair_input')
     ...
     ! write(6,*)'number of atoms in molecule:',natoms
      ...
      allocate(cosphi(npair,nmovie))
      allocate(pm(npair,nmovie))
      allocate(r12(npair,nmovie))
      allocate(r13(npair,nmovie))
      allocate(r23(npair,nmovie))
...
!===============================关键部分===============================
      ...
      do k=1,npair
        ...
        do j =1, nmovie
          h(k,j)=0
          h_d(k,j)=0 
          r13(k,j)= (x(m1,j)-x(m3,j))**2+  &
                    (y(m1,j)-y(m3,j))**2+  &
                    (z(m1,j)-z(m3,j))**2      !r:squra of distances
          r12(k,j)= (x(m1,j)-x(m2,j))**2+  &
                    (y(m1,j)-y(m2,j))**2+  &
                    (z(m1,j)-z(m2,j))**2
          r23(k,j)= (x(m2,j)-x(m3,j))**2+  &
                    (y(m2,j)-y(m3,j))**2+  &
                    (z(m2,j)-z(m3,j))**2
          pm(k,j)= (x(m3,j)-x(m2,j))*   &
                   (x(m1,j)-x(m2,j))+   & 
                   (y(m3,j)-y(m2,j))*   & 
                   (y(m1,j)-y(m2,j))+   & 
                   (z(m3,j)-z(m2,j))*   &
                   (z(m1,j)-z(m2,j)) 
          cosphi(k,j)= pm(k,j)/(sqrt(r23(k,j)*r12(k,j)))            !pm: point multiplication.
          if (r13(k,j) .lt. rohc .and. r12(k,j).lt.rooc          & 
             .and. cosphi(k,j).gt. cosphic) then    
              h(k,j)=1.0 
              hb(k)=hb(k)+h(k,j)                          
          endif
          if (r13(k,j) .lt. rohc ) then                           
              h_d(k,j)=1.0
...
!Write the result
!================
      open(10,file='achbond_1pair_nhb.dat')
...
!C_HB(t) and C_HB_d(t)     
!======================
      open(10,file='achbond_1pair_acf_h.dat')
      ...
      open(10,file='achbond_1pair_acf_h_d.dat')
        do i=1,int(nmovie*2/5)
            write(10,*)i-1,corr_h_d(i)
        enddo
        write(6,*)'written in achbond_1pair_acf_h_d.dat'
      close(10)
...
!==========================
      open(10,file='achbond_1pair_ln_acf_h.dat')
        do i=1,int(nmovie*2/5)
            write(10,*)i-1,log(corr_h(i))
        enddo
        write(6,*)'written in achbond_1pair_ln_acf.dat'
      close(10)

      open(10,file='achbond_1pair_ln_acf_h_d.dat')
        do i=1,int(nmovie*2/5)
            write(10,*)i-1,log(corr_h_d(i))
        enddo
        write(6,*)'written in achbond_1pair_ln_acf_h_d.dat'
      close(10)
