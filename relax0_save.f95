...
!  We find that the auto correlation function is alway a positive value, 
!  idicating that the O-H bond has never breaking in such a time period.  
!=======================================================================
...
program  relaxiation 
  ...
  integer :: i,iatom,imovie,mt,natoms,nhmaxt,nmovie,nts
  integer :: nbonds
  ...
  !Initialization
  i=0 ; iatom=0 ; imovie=0 ; mt=0 ; natoms=0
  nmovie=0 ; nts=0
  corr(:)=0.d0
  ...
  do mt=0,nmovie-1!Loop of Dt (=t-t0)
     ...
     corr(mt+1) = corr(mt+1)/dble(nmovie-mt)!Normalization by the number of time steps
  end do
