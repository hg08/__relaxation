...
!==================
      ...
      write(6,*)'What is the distance of r_O-hal(r_shell1):'
      read(5,*)r_shell1
...
!========================================
      ...
      enddo
      i_OW_shell1_hal=i
      allocate(ndx_OW_shell1_hal(i_OW_shell1_hal)) 
      i=0
      do i1=1, i_OW
          m1=ndx_OW(i1)
          do i2=1, i_I 
              ...
              do j =1,1
                  ...
                  if (r<r_shell1) then
                    i=i+1 
                    ndx_OW_shell1_hal(i)=m1
                  endif
              enddo
          enddo
      enddo
!========================================
!Print index of O in the shell of 'I-wat'
!========================================
      i=0
      do i1=1, i_OW
          m1=ndx_OW(i1)
          do i2=1, i_OW_shell1_hal
              m2=ndx_OW_shell1_hal(i2)
                  do j =1,1
                      r= (x(m2,j)-x(m1,j))**2+  &
                           (y(m2,j)-y(m1,j))**2+  &
                           (z(m2,j)-z(m1,j))**2
                  if (r<rooc .and. r > rooc_min) then
                    i=i+1 
                  endif
                  enddo
          enddo
      enddo
      i_OW_shell2_hal=i
      i_OW_shell_hal=i_OW_shell2_hal+i_OW_shell1_hal
      allocate(ndx_OW_shell_hal(i_OW_shell_hal)) 
      !===================
      !Make ndx_OW_shell_hal
      !===================
      ndx_OW_shell_hal(:)=0
      do i =1, i_OW_shell1_hal
          ndx_OW_shell_hal(i)=ndx_OW_shell1_hal(i)
      enddo 
      i=0
      do i1=1, i_OW
          m1=ndx_OW(i1)
          do i2=1, i_OW_shell1_hal 
              ...
              do j =1,1
                  ...
                  if (r<rooc .and. r > rooc_min) then
                    i=i+1 
                    ndx_OW_shell_hal(i_OW_shell1_hal + i)=m1
                  endif
              enddo
          enddo
      enddo
...
!=====================================
      open(20,file=trim(filename)//'_shell_list.dat')
      do i1=1, i_OW_shell_hal-1! No O atom can not be bonded to itself 
          m1=ndx_OW_shell_hal(i1)
          do i2=i1+1, i_OW_shell_hal 
              m2=ndx_OW_shell_hal(i2)
              do ii=1,i_H
                  m3=ndx_H(ii)
                  do j =1,1
                      r23= (x(m2,j)-x(m3,j))**2+  &
      ...
      enddo
      close(20)
      deallocate(ndx_OW,ndx_OW_shell1_hal,ndx_OW_shell_hal,ndx_I,&
                 ndx_H,x,y,z)
!==============================================================
