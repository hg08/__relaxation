...
real(kind=rk),allocatable,dimension(:) :: corr ! the correlation function k_in(t)
real(kind=rk),allocatable,dimension(:) :: corr_n, corr_n2 ! the correlation function n(t)
real(kind=rk) :: scalar
real(kind=rk) :: scalar_n ! for n(t)
real(kind=rk) :: scalar_n2 ! for n(t)
...
!Smoothing k(t)
!=============
CALL moving_window_average(corr,n_neighbors)

!====================================
! The second method to calculate n(t)
!====================================
...
do mt=0,nmo-1! time interval
    ...
    !-----------------------------------------------------------
    scalar_n2=0.d0 ! Calculate n(t)=\int_0^t dt' k_in(t'), donoted by prob_in_h_d
    do jj=1, nmo-1
        scalar_n2=scalar_n2+corr(jj)*delta_t
    enddo
    corr_n2(mt+1)=scalar_n2/(nmo-1)
enddo
...
!========================================================================
open(10,file=trim(filename)//'_rf_in.dat')
    do i=1,int(nmo*rate)                
        write(10,*)(i-1)*delta_t,corr(i)
    enddo
    write(6,*)'written in'//trim(filename)//'_rf_in.dat'
close(10)
! write out n(t)
...
close(10)
! write out n(t) from the second method
open(10,file=trim(filename)//'_n2.dat')
    do i=1,int(nmo*rate)                
        write(10,*)(i-1)*delta_t,corr_n2(i)
    enddo
    write(6,*)'written in'//trim(filename)//'_n2.dat'
close(10)
!-----
! log_X
!-----
! write out log[k_in(t)]
open(10,file=trim(filename)//'_log_rf_in.dat')
    do i=1,int(nmo*rate)                
        write(10,*)(i-1)*delta_t,log(corr(i))
    enddo
    write(6,*)'written in'//trim(filename)//'_log_rf_in.dat'
close(10)
! write out log[n(t)]
...
close(10)
!------------------------------------------------
! write out logn(t) from the second method
! NOT DO YET
! THERE WILL BE A PROBLEM for the result, because
! n(t) is negative for some t
!------------------------------------------------
!=============
!test the traj
!=============
...
!==============================================================
deallocate(x,y,z)
deallocate (h,dh,corr,corr_n,corr_n2,h_d)

! Count time
call system_clock(end_time,rat)
write(6, *)"elapsed time: ", real(end_time-begin_time)/real(rat) 

END PROGRAM ghbrf_kpair_in_pbc 
