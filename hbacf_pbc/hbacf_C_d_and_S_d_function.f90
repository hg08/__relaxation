...
!===================================================
      program hbacf_C_d_and_S_d_function
      implicit none
!==========
!parameters
!==========
      ...
      real(kind=rk),allocatable,dimension (:)    :: corr_h_d, &
                                                    corr_hh_d
      real(kind=rk)  :: scalar_h_d, scalar_hh_d 
...
!==================================
        do mt=0,nmo-1     ! time interval
            if (hb(k)>hb_min) then
                ...
                do j=1, nmo-mt-1
                    ...
                    scalar_hh_d=scalar_hh_d+h(j)*hh_d        
                enddo
                scalar_h_d=scalar_h_d/(nmo-mt-1)        ! C_k(t)
                scalar_hh_d=scalar_hh_d/(nmo-mt-1)      ! S_k(t)
                corr_h_d(mt+1)=corr_h_d(mt+1)+scalar_h_d    ! sum_C_k(t)
                corr_hh_d(mt+1)=corr_hh_d(mt+1)+scalar_hh_d    ! sum_S_k(t)
            endif
        enddo
      enddo! k-loop 
      tot_hb=tot_hb/np
      ...
      do mt=0,nmo-1! time interval
          ...
          corr_hh_d(mt+1)=corr_hh_d(mt+1)/(np*tot_hb)  
      enddo
...
!========================
      ...
      open(10,file=trim(filename)//'_achbond_hh_d.dat')
        do i=1,int(nmo*rate)
            write(10,*)(i-1)*delta_t,corr_hh_d(i)
        enddo
        write(6,*)'written in '//trim(filename)//&
                  '_achbond_hh.dat'
      close(10)
...
!================================
      ...
      open(10,file=trim(filename)//'_achbond_ln_hh_d.dat')
        do i=1,int(nmo*rate)
            write(10,*)(i-1)*delta_t,log(corr_hh_d(i))
        enddo
        write(6,*)'written in '//trim(filename)//&
                  '_achbond_ln_hh_d.dat'
      close(10)
!===========
!Print <h> 
!===========      
